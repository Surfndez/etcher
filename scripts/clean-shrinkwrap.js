 ...
 *
 * @param {*} keys - Keys reference
 * @param {*} tree - Tree to iterate over
 * @param {*} onNode - Function to apply
 * @param {*} parentName - Name of dependent
 * @param {*} depType - Dependency type (dependencies|requires)
 *
 * @example
 * removeDependency(keys, tree, onNode, parentName)
 */
const removeDependency = (keys, tree, onNode, parentName, depType) => {
  let name = null
  for (let index = 0; index < keys.length; index += 1) {
    ...
    if (tree[depType][name]) {
      onNode(tree, parentName || tree.name, name, tree[depType][name], depType)

      // Check that the walking function didn't remove the dependency
      // if so, skip it and continue with the next one
      if (tree[depType][name] && tree[depType][name][depType]) {
        traverseDeps(tree[depType][name], onNode, name)
      }
    }
  }
}
...
const traverseDeps = (tree, onNode, parentName) => {
  if (!tree.dependencies && !tree.requires) {
    return
  }

  const depsKeys = Object.keys(tree.dependencies)
  removeDependency(depsKeys, tree, onNode, parentName, 'dependencies')

  if (_.isObject(tree.requires)) {
    const reqsKeys = Object.keys(tree.requires)
    removeDependency(reqsKeys, tree, onNode, parentName, 'requires')
  }
}
...
traverseDeps(shrinkwrap, (parent, parentName, name, info, depType) => {
  ...
  if (packageInfo.platformSpecificDependencies.includes(name)) {
    ...
    // Check that we're not in the root object
    // before removing the "requires" property
    if (_.isObject(parent.requires)) {
      parent.requires[name] = undefined
      Reflect.deleteProperty(parent.requires, name)
    }
    return
  }
